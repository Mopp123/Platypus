...
namespace platypus
{
    ...
    static void load_gltf_animations_NEW(
        tinygltf::Model& gltfModel
    )
    {
        // Need association with: node, keyframe(time) AND data (rotation or translation)
        ...
        for (tinygltf::Animation& gltfAnimation : gltfModel.animations)
        {
            for (tinygltf::AnimationChannel& channel : gltfAnimation.channels)
            {
                ...
                // Should be the actual component count, meaning how many vectors or quaternions, INDEPENDENT from struct components
                ...
                if (path == "translation")
                {
                    size_t valueCount = count;
                    std::vector<Vector3f> translations(valueCount);
                    memcpy((void*)translations.data(), pAnimData, valueCount * sizeof(Vector3f));
                    // How the hell do we have 12 translations for 4 keyframes???
                    Debug::log("___TEST___Node: " + std::to_string(targetNode) + ": Marked " + std::to_string(translations.size()) + " translations for " + std::to_string(keyframes.size()) + " keyframes");
                ...
                {
                    size_t valueCount = count;
                    std::vector<Quaternion> rotations(valueCount);
                    memcpy((void*)rotations.data(), pAnimData, valueCount * sizeof(Quaternion));
                    Debug::log("___TEST___Node: " + std::to_string(targetNode) + ": Marked " + std::to_string(rotations.size()) + " rotations for " + std::to_string(keyframes.size()) + " keyframes");
                }
            }
        }
        PLATYPUS_ASSERT(false);
    ...
    {
        // TESTING:
        load_gltf_animations_NEW(gltfModel);
        PLATYPUS_ASSERT(false);
        // ---
