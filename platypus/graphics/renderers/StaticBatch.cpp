...
namespace platypus
{
    static std::vector<Buffer*> get_or_create_transforms_buffers(
        Batcher& batcher,
        ID_t identifier,
    ...
    )
    {
        ...
        {
            Debug::log(
                "@get_or_create_transforms_buffers "
    ...
    )
    {
        ...
        pBatch->dynamicVertexBuffers[0] = get_or_create_transforms_buffers(
            batcher,
            identifier,
            pBatch->maxLength,
            framesInFlight
    ...
    )
    {
        ...
        for (Batch* pBatch : batcher.getBatches(batchID))
        {
            ...
            {
                BatchShaderResource* pResource = batcher.getSharedBatchResource(batchID, 0);
                Buffer* pInstancedBuffer = pResource->buffer[currentFrame];
                pInstancedBuffer->updateHost(
                    (void*)&transformationMatrix,
                    sizeof(Matrix4f),
                    sizeof(Matrix4f) * pBatch->instanceCount
                );
                pResource->requiresDeviceUpdate = true;
