...
namespace platypus
{
    ...
    {
        std::vector<Mesh*> createdMeshes;
        size_t extensionBegin = filepath.find('.');
        std::string extension = filepath.substr(extensionBegin + 1, filepath.size() - extensionBegin);
        if (extension != "png")
        {
            std::vector<MeshData> loadedMeshes;
            if (!load_gltf_model(filepath, loadedMeshes, outAnimations))
        ...
        {
            Image* pHeightmap = Image::load_image(filepath, ImageFormat::R8_UNORM);
            size_t w = pHeightmap->getWidth();
            size_t h = pHeightmap->getHeight();
            std::vector<float> heights(w * h);
            const float heightModifier = 5.0f; // TODO: Allow specifying this + tile width
            for (size_t y = 0; y < h; ++y)
            {
                for (size_t x = 0; x < w; ++x)
                {
                    float height = static_cast<float>(pHeightmap->getColorChannelValue(x, y, 0) * heightModifier) / 255.0f * heightModifier;
                    heights[x + y * w] = height;
                }
            }

            createdMeshes.push_back(
                createTerrainMesh(
                    1.0f, // tile width
                    heights,
                    false, // dynamic?
